...
<body class="bg-gradient-to-br from-indigo-50 via-white to-pink-50 text-slate-900 min-h-screen">
    ...
    <script type="module">
        ...
        const generateNDAText = (data) => {
            const selectedScopes = SCOPE_OPTIONS.filter(o => data.scope?.[o.id]).map(o => `「${o.label}」`);
        ...
        const SignaturePad = ({ onSave, label, existingSignature, disabled, accentColor }) => {
            ...
            return React.createElement('div', { className: 'flex flex-col gap-3' },
                ...
                React.createElement('div', { className: `relative border-2 ${disabled ? 'border-indigo-50 bg-indigo-50/10' : 'border-slate-100 bg-white hover:border-indigo-200'} rounded-[2rem] overflow-hidden aspect-[4/3] md:aspect-[3/2] shadow-sm transition-all` },
                    disabled ? React.createElement('img', { src: existingSignature, className: 'w-full h-full object-contain p-6' }) :
        ...
        function App() {
            ...
            return React.createElement('div', { className: 'max-w-7xl mx-auto px-4 py-8' },
                React.createElement('header', { className: 'flex flex-col md:flex-row justify-between items-center mb-10 bg-white/70 backdrop-blur-md p-6 rounded-[2.5rem] shadow-xl border border-white gap-4' },
                    React.createElement('div', { onClick: () => setView('home'), className: 'cursor-pointer group flex flex-col items-center md:items-start' },
                        ...
                        React.createElement('div', { className: 'flex items-center gap-1.5 mt-2 bg-slate-50 px-3 py-1 rounded-full text-[10px] font-black text-slate-400' },
                            React.createElement(Zap, { size: 12, className: 'text-amber-400 fill-amber-400' }), `今までに ${count}回 結ばれたよ！`)
                    ),
                    React.createElement('nav', { className: 'flex gap-2 bg-slate-100/50 p-1.5 rounded-3xl' },
                        [ {id:'home', label:'ホーム'}, {id:'history', label:`履歴 (${filtered.length})`} ].map(n => React.createElement('button', {
                ...
                view === 'home' ? React.createElement('div', { className: 'grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto' },
                    ...
                    React.createElement('div', { className: 'bg-white p-10 rounded-[3rem] shadow-xl border border-slate-50 text-center flex flex-col items-center gap-6 hover:-translate-y-1 transition-all' },
                        ...
                        React.createElement('div', { className: 'w-full flex gap-2 p-2 bg-slate-50 rounded-[2rem] border border-slate-100' },
                            React.createElement('input', { type: 'text', placeholder: 'IDを入力...', className: 'flex-grow bg-transparent px-4 py-2 font-bold outline-none text-sm', value: searchId, onChange: e => setSearchId(e.target.value) }),
                            React.createElement('button', { onClick: handleSearch, className: 'bg-slate-900 text-white px-6 rounded-[1.5rem] font-bold text-sm hover:bg-black transition-all' }, 'GO!')
                        )
                    )
                ) : 
                view === 'edit' ? React.createElement('div', { className: 'grid grid-cols-1 lg:grid-cols-2 gap-10' },
                    React.createElement('div', { className: 'space-y-6' },
                        React.createElement('div', { className: 'bg-white p-8 rounded-[2.5rem] shadow-xl border border-slate-50 space-y-6' },
                            React.createElement('h2', { className: 'text-xl font-black flex items-center gap-2' }, React.createElement('div', { className: 'w-1.5 h-6 bg-indigo-500 rounded-full' }), 'Setting'),
                            currentNdaId && React.createElement('div', { className: 'bg-indigo-600 p-4 rounded-2xl flex justify-between items-center text-white shadow-inner' },
                                React.createElement('div', { className: 'truncate' }, React.createElement('p', { className: 'text-[8px] font-black opacity-50 uppercase tracking-widest' }, 'Contract ID'), React.createElement('p', { className: 'text-xs font-mono' }, currentNdaId)),
                        ...
                        React.createElement('div', { className: 'bg-white p-8 rounded-[2.5rem] shadow-xl border border-slate-50 space-y-6' },
                            React.createElement('h2', { className: 'text-xl font-black flex items-center gap-2' }, React.createElement('div', { className: 'w-1.5 h-6 bg-pink-500 rounded-full' }), 'Signature'),
                            React.createElement('div', { className: 'grid grid-cols-2 gap-4' },
                ...
                showSuccess && React.createElement('div', { className: 'fixed inset-0 z-50 flex items-center justify-center bg-emerald-900/60 backdrop-blur-xl animate-in fade-in duration-300' },
                    React.createElement('div', { className: 'bg-white p-10 rounded-[3rem] shadow-2xl text-center max-w-sm w-full space-y-6 animate-in zoom-in-95' },
                        React.createElement('div', { className: 'w-16 h-16 bg-emerald-500 rounded-2xl mx-auto flex items-center justify-center text-white shadow-xl animate-bounce-subtle' }, React.createElement(PartyPopper)),
                        ...
                        React.createElement('button', { onClick: () => { setShowSuccess(false); setView('home'); }, className: 'w-full bg-slate-900 text-white font-black py-4 rounded-[2rem] shadow-lg hover:bg-black transition-all' }, 'ホームに戻る')
                    )
                )
            );
        }
